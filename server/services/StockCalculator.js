...
export function casesToBottles(qty, packOf) {
    if (!packOf || packOf <= 0) return Math.round(parseFloat(qty) || 0);
    const q = parseFloat(qty) || 0;
    const cases = Math.floor(q);
    const bottles = Math.round((q - cases) * 100);
    return (cases * packOf) + bottles;
}
...
export function bottlesToCases(totalBottles, packOf) {
    ...
    const cases = Math.floor(totalBottles / packOf);
    const rem = totalBottles % packOf;
    return `${cases}.${String(rem).padStart(2, '0')}`;
}

// Helper: remainingQty is String in DB — always parse before any comparison
function rQty(item) {
    return parseFloat(item.remainingQty) || 0;
}
...
class StockService {
    ...
    static async cleanupExpiredItems(depoId) {
        ...
        for (const purchase of purchases) {
            ...
            purchase.items = purchase.items.filter(item => {
                // ✅ parseFloat — remainingQty is String
                if (item.expiryDate <= now && rQty(item) > 0) {
                    expiredLog.push({
                        itemCode: item.itemCode,
                        qty: item.remainingQty,
                        expiryDate: item.expiryDate,
            ...
            const allExhausted =
                purchase.items.length === 0 ||
                // ✅ parseFloat on every remainingQty comparison
                purchase.items.every(i => rQty(i) <= 0);
    ...
    static async processLoadout(loadoutItems, depoId) {
        ...
        for (const loadoutItem of loadoutItems) {
            ...
            const requestedBottles = casesToBottles(loadoutItem.qty, packOf);
            let remainingToLoad = requestedBottles;
            const allocations = [];

            // ✅ Removed 'items.remainingQty': { $gt: 0 } from query
            // String field can't be compared with number in MongoDB
            const purchases = await PurchaseItemwise.find({
                depo: depoId,
                isFullyProcessed: false,
                'items.itemCode': code,
            ...
            for (const purchase of purchases) {
                ...
                for (const item of purchase.items) {
                    if (
                        item.itemCode.toUpperCase() === code &&
                        rQty(item) > 0 &&          // ✅ parseFloat comparison
                        item.expiryDate > now
                    ) {
                        const availableBottles = casesToBottles(item.remainingQty, packOf);
                        ...
                        // Save back as cases.bottles string
                        item.remainingQty = bottlesToCases(bottlesLeft, packOf);
                        ...
                        allocations.push({
                            ...
                            purchaseDate: purchase.date,
                            expiryDate: item.expiryDate,
                            qty: bottlesToCases(bottlesToDeduct, packOf),
                            qtyBottles: bottlesToDeduct
                        });

                        // ✅ parseFloat on every remainingQty
                        if (purchase.items.every(i => rQty(i) <= 0)) {
                            purchase.isFullyProcessed = true;
                        }
            ...
            results.push({
                itemCode: code,
                requestedQty: loadoutItem.qty,
                allocatedQty: bottlesToCases(allocatedBottles, packOf),
                shortfall: remainingToLoad,              // 0 = fully allocated ✅
                shortfallCases: bottlesToCases(remainingToLoad, packOf),
                allocations
            });
        }
    ...
    static async getCurrentStock(depoId) {
        ...
        for (const purchase of purchases) {
            for (const item of purchase.items) {
                // ✅ parseFloat — can't compare String with 0 directly
                if (rQty(item) <= 0 || item.expiryDate <= now) continue;
    ...
    static async getExpiringItems(depoId, daysThreshold = 30) {
        ...
        // ✅ Removed 'items.remainingQty': { $gt: 0 } — String vs Number won't work in MongoDB
        const purchases = await PurchaseItemwise.find({
            depo: depoId,
            isFullyProcessed: false,
            'items.expiryDate': { $gt: now, $lte: thresholdDate }
